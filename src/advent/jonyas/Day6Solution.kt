...
private fun indexForCircularIntArray(array: IntArray, index: Int): Int {
  ...
  return when (index) {
    in 0 until array.size -> index
    else -> index % array.size
  }
}

private fun findMaxItemLocation(): (IntArray) -> Int {
  return { array ->
    // Assuming there are always items
    array.max()!!.let { maxItem -> array.indexOfFirst { it == maxItem } }
  }
}

private fun findInfiniteLoopIteration(memory: IntArray,
                                      selector: (IntArray) -> Int,
                                      distributeItems: (IntArray, Int) -> Unit):
    // First being the amount of iterations, second iterations from infinite loop to end
    Pair<Int, Int> {
  ...
  while (!stateSet.contains(memory.contentToString())) {
    // save current state
    stateSet.add(memory.contentToString())
    // Distribute items
    distributeItems(memory, selector(memory))
    // Increase counter
    iteration++
  }
 ...
 */
private fun part1And2(fileName: String) {

  val numbers = File(fileName).readLines()
      .first()
      .split("\t")
      .map { it.toInt() }
      .toIntArray()
